...
function checkWord() {
  var wordTemp = split(word, "");
  var ok = false;
  var i = 0
  while (ok == false) {
    if (wordTemp[wordTemp.length - 1] == "S") {
      i++;
      word = wordnikWord[i].word.toUpperCase();
      wordTemp = split(word, "");
    } else {
      ok = true;
    }
...
function getWord() {
  var path = "https://api.wordnik.com/v4/words.json/randomWords?hasDictionaryDef=true&includePartOfSpeech=noun&excludePartOfSpeech=noun-plural&minCorpusCount=10000&maxCorpusCount=-1&minDictionaryCount=1&maxDictionaryCount=-1&minLength=5&maxLength=-1&limit=10&api_key=";
